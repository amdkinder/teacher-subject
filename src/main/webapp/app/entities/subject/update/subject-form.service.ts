...
type SubjectFormDefaults = Pick<NewSubject, 'id' | 'inUse'>;

type SubjectFormGroupContent = {
  id: FormControl<ISubject['id'] | NewSubject['id']>;
  name: FormControl<ISubject['name']>;
  hours: FormControl<ISubject['hours']>;
  inUse: FormControl<ISubject['inUse']>;
  spec: FormControl<ISubject['spec']>;
};
...
export class SubjectFormService {
  createSubjectFormGroup(subject: SubjectFormGroupInput = { id: null }): SubjectFormGroup {
    ...
    return new FormGroup<SubjectFormGroupContent>({
      ...
      hours: new FormControl(subjectRawValue.hours),
      inUse: new FormControl(subjectRawValue.inUse),
      spec: new FormControl(subjectRawValue.spec),
    });
  }
