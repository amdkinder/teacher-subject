...
import { EntityArrayResponseType, SpecService } from '../service/spec.service';
...
export class SpecComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISpec): number => this.specService.getSpecIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.specs = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISpec[] | null): ISpec[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.specService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
